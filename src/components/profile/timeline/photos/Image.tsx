...
const ImageLoader2 = () => {
    return(
        <div role="status" className="flex items-center justify-center min-h-[130px] max-h-[130px] max-w-sm bg-gray-300 rounded-lg animate-pulse dark:bg-gray-700">
...
const Image = ({data}:PropsImage) => {
    ...
    return(
        <div className={`col-span-1 border-[3px] border-gray-300 min-h-[140px] max-h-[140px] p-1`}>
            <img className={`w-full h-full ${!isLoaded && 'hidden'}`} src={data?.url} alt="" onLoad={handleImageLoad}/>
            {
                !isLoaded  && <ImageLoader2/>
            }
        </div>
    )
}
